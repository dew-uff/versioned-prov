...
with prov.desc("L3 - list definition / assign", line=3):
    with prov.desc("L3 - list definition"):
        ...
        prov_label = ["2", "m", "0"]
        e_list, rows = prov.define_array("list", lastrow, prov_label, "list", prov.hadMember, show1=False, attrs=SPECIFIC)
    ...
    with prov.desc("L3 - assign"):
        e_lastrow = prov.entity("lastrow", repr(lastrow), "name", "lastrow", show1=True)
...
with prov.desc("L4 - list definition / assign", line=4):
    ...
    with prov.desc("L4 - list definition"):
        ...
        e_list, rows = prov.define_array("matrix", dist, prov_label, "list", prov.hadMember, show1=False, attrs=SPECIFIC)
    ...
    with prov.desc("L4 - assign"):
        e_dist = prov.entity("dist", repr(dist), "name", "dist", show1=True)
        prov.had_members(e_dist, row_ents, attrs=SPECIFIC)
        prov.activity("assign", [(e_dist, e_list)], attrs=HIDE)

        e_result = prov.entity("result", repr(result), "name", "result", show1=True)
        ...
        for j in indexes:
            ...
            if disti[j] > ikj:
                ...
                with prov.desc("L21 - part assign with propagation", line=21):
                    ...
                    new_e_distk = e_distk
                    if i == k:
                        new_e_distk = prov.entity("distk", repr(distk), "name", "distk", show1=True, attrs=SPECIFIC)
                    ...
                    if i == nodes - 1:
                        new_e_lastrow = prov.entity("lastrow", repr(lastrow), "name", "lastrow", show1=True, attrs=SPECIFIC)
                    ...
                    new_e_result = prov.entity("result", repr(result), "name", "result", show1=True, attrs=SPECIFIC)
...
with prov.desc("L22 - access / access / call", line=22):
    e_result_a0 = prov.entity("result@0", repr(result[0]), "access", "result[0]")
