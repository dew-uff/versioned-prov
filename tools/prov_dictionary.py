from provn import graph, _arrow2, _arrow3, _labeldict
...
def had_dictionary_member(dot, did=None, eid=None, key=None, attrs=None, id_=None):
    if key and key.startswith('"') and key.endswith('"'):
        key = key[1:-1]
    return _arrow2(dot, did, eid, "[{}]".format(key), extra={"color": "darkgreen"})
...
def derived_by_insertion_from(dot, dgen=None, duse=None, changes=None, attrs=None, id_=None):
    ...
    if duse:
        ...
        result += "\n" + dot.arrow(
            point, use_url, attrs=dict(**_labeldict("der"), **{"color": "darkgreen"})
        )
    ...
    for key, ent in changes:
        ...
        if ent:
            ...
            result += "\n" + dot.arrow(
                point, ent_url, attrs=dict(**_labeldict("[{}]".format(key)), **{"color": "darkgreen"})
            )
    return result
...
def derived_by_insertion_from(dot, dgen=None, duse=None, removals=None, attrs=None, id_=None):
    ...
    return _arrow2(dot, dgen, duse, "der-rem\n{}".format(tremovals), extra={"color": "darkgreen"})
